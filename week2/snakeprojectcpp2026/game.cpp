// game.cpp
// 전체 게임 루프, 입력 처리, 스테이지 전환, 점수 저장과 화면 UI 출력을 구현한다.
...
bool Game::update()
{
    // 게임 시작 후 경과한 초마다 1점씩 누적한다.
    {
        using namespace std::chrono;
        const int elapsed = static_cast<int>(
            duration_cast<seconds>(steady_clock::now() - gameStartedAt_).count());
        while (lastTimeScoreSecond_ < elapsed)
        {
            score_.addPoints(1);
            ++lastTimeScoreSecond_;
        }
    }
    ...
    // 무적 아이템 효과 만료 확인.
    if (itemInvincibleActive_ &&
        std::chrono::steady_clock::now() >= itemInvincibleUntil_)
    {
        itemInvincibleActive_ = false;
    }

    // 치트 무적 또는 아이템 무적이면 벽 충돌을 생존한다.
    const bool wallSurvive = invincible_ || itemInvincibleActive_;
             ...
             map_.isWall(next.first, next.second))
    {
        if (wallSurvive)
        {
            // 무적 상태에서는 벽에 부딪혀도 죽지 않고 제자리에 머문다.
        ...
        snake_.hitsSelf(next))
    {
        if (invincible_)
        {
            // 치트 무적 모드에서는 자기 몸 충돌도 제자리에 머무는 방식으로 무시한다.
            score_.updateLength(snake_.length());
            return advanceStageIfComplete();
        }
    ...
    // 다음 칸에 팩맨이 있으면 충돌 — 치트 무적일 때만 생존한다.
    if (next == pacman_.position())
    {
        if (!invincible_)
        {
            return false;
    ...
    else if (consumed && type == Item::Type::Invincible)
    {
        // 5초간 벽 충돌 무적 효과를 부여한다.
        itemInvincibleActive_ = true;
        itemInvincibleUntil_ = std::chrono::steady_clock::now() +
                                std::chrono::seconds(ItemManager::InvincibleEffectSeconds);
        statusMessage_ = "Invincible Shield: 5 seconds!";
    }
...
void Game::startStage()
{
    ...
    // 스테이지 초기화 시 포탈 사용 중 상태와 아이템 무적 상태를 초기화한다.
    portalManager_.setSnakeUsingPortal(false);
    itemInvincibleActive_ = false;
    initStageObjects();
...
void Game::initStageObjects()
{
    portalManager_.generate(map_);
    itemManager_.initStage(map_, snake_.body());
    lastRegeneration_ = std::chrono::steady_clock::now();
}
...
void Game::draw(WINDOW* window) const
{
    ...
    map_.draw(window, snake_.body(), itemManager_.items(), snake_.length(), pacman_.position());
    ...
    if (itemInvincibleActive_)
    {
        const auto remaining = std::chrono::duration_cast<std::chrono::milliseconds>(
            itemInvincibleUntil_ - std::chrono::steady_clock::now()).count();
        const double secs = remaining > 0 ? remaining / 1000.0 : 0.0;
        mvwprintw(window, 3, GameMap::DrawWidth + 4, "Shield: %.1fs  ", secs);
    }
    else
    {
        mvwprintw(window, 3, GameMap::DrawWidth + 4, "Shield: --    ");
    }

    score_.draw(window, 4, GameMap::DrawWidth + 4, highScore_);
...
{
    if (score_.totalScore() > highScore_)
    {
        highScore_ = score_.totalScore();
...
void Game::showGameOver(WINDOW* window) const
{
    ...
    mvwprintw(window, GameMap::Size / 2 + 2, GameMap::DrawWidth / 2 - 10,
              "Score        : %d", score_.totalScore());
    mvwprintw(window, GameMap::Size / 2 + 3, GameMap::DrawWidth / 2 - 10,
              "Best Score   : %d", highScore_);
    mvwprintw(window, GameMap::Size / 2 + 5, GameMap::DrawWidth / 2 - 14,
...
void Game::showGameClear(WINDOW* window) const
{
    ...
    mvwprintw(window, GameMap::Size / 2 + 2, GameMap::DrawWidth / 2 - 10,
              "Best Score : %d", highScore_);
    mvwprintw(window, GameMap::Size / 2 + 4, GameMap::DrawWidth / 2 - 14,
              "Press any key to exit");
    wrefresh(window);
    nodelay(stdscr, FALSE);
    getch();
